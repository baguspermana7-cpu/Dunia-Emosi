...
(function (global) {
  ...
  /* v61.8 — the SHARED film files: games/film/gg-paint.js and everything under
     games/film/assets/ (the "Pilih Pengejar" portraits, the stickers). They are
     loaded by film-play.html, they live outside every slug folder, and so they
     were covered by NO manifest and downloaded by NO installer. Every game
     reported "Siap offline" and then died with no network — match-up on one
     missing script, gotham on ten missing portraits. The list is generated by
     tools/build-film-manifests.mjs rather than hand-kept here, because a
     hand-kept list is exactly what went stale in the first place. */
  var SHARED_MANIFEST = 'film/shared-manifest.json'
  ...
  function install(slug, onProgress) {
    ...
    var promise = (function () {
      var mf, cache, sharedAssets = [], persisted = false
      ...
      return swReady()
        ...
        .then(function (m) {
          mf = m
          // The shared wrapper files. A failure here must NOT abort the install:
          // an older deploy has no shared-manifest.json, and a game that
          // installs without the shared portraits is still better than a game
          // that refuses to install at all.
          return fetch(new URL(SHARED_MANIFEST, gamesBase()).href, { cache: 'no-cache' })
            .then(function (r) { return r.ok ? r.json() : null })
            .catch(function () { return null })
        })
        .then(function (sm) {
          sharedAssets = (sm && sm.assets) || []
        ...
        .then(function (r) {
          ...
          for (var si = 0; si < sharedAssets.length; si++) {
            var sa = sharedAssets[si]
            shellJobs.push({
              url: new URL('film/' + sa.p, gamesBase()).href,
              size: sa.s, p: 'shared/' + sa.p, shell: true
            })
          }
