...
### L110 — An SVG wheel spins on its axle only with an EXPLICIT viewBox-unit origin + spokes (game.js v55.70)
- **Symptom**: museum loco wheels "revolution mutar tanpa poros" — every circle (wheels, smoke, headlight) orbited the centre of the SVG instead of each wheel rotating in place.
- **Root cause(s)**: (1) a blanket `svg circle{transform-origin:center;animation:spin}` rule rotated ALL circles, and (2) it (and the shared `.train-wheel` rule) relied on `transform-box:fill-box` to make `transform-origin:center` mean "this element's centre". The owner's tablet webview doesn't honour `fill-box`, so `center` resolved to the **viewBox centre** → orbit. Also a plain `<circle>` is radially symmetric, so even a correct spin is invisible.
- **Fix**: a `dunWheel()` helper renders each wheel as a `<g>` with rim + hub + **spokes** (visible rotation) and an inline `transform-origin:<cx>px <cy>px` in **viewBox units** — no `fill-box` dependency, correct axle on every browser. Target only the wheels, never a blanket `circle` selector.
- **Lesson**: for SVG rotation, don't trust `transform-box:fill-box` on old webviews — set `transform-origin` to the element's actual coordinates in viewBox space. And never animate a bare symmetric `<circle>` and expect to SEE spin — add spokes. Scope the animation to the wheel group, not every `<circle>` in the SVG.

### L111 — vtracer flattens alpha; key-colour + path-strip gives a transparent vector cutout (bg pipeline v55.71)
- **Symptom**: tracing a city-band PNG with a transparent sky produced an SVG whose sky was OPAQUE (vtracer fills a full-canvas base under everything) — the band would have covered the procedural sky.
- **Root cause**: vtracer ignores the alpha channel; transparent pixels trace as whatever RGB they carry.
- **Fix**: before tracing, paint the knocked-out sky a unique KEY colour (e.g. `#FD00FE`); trace with `hierarchical='cutout'`; then post-process the SVG to DELETE every `<path>` whose fill ≈ the key → the sky becomes genuinely empty/transparent. Keep a real-alpha WebP as the raster fallback separately.
- **Lesson**: when a vectoriser can't carry transparency, encode "transparent" as a sentinel colour and strip it from the output. Also: gate optional per-level asset fetches behind a shipped `index.json` so levels without an asset never 404 (a 404 logs a console error, failing the clean-console gate). And cache a detailed SVG to a RenderTexture/Sprite ONCE → its per-frame cost is one draw call regardless of polygon count.

### L112 — Raster beats vector for PAINTERLY backgrounds, on BOTH responsiveness and fidelity (A-312 v55.72)
- **Decision**: the owner wanted reference-accurate backdrops "super responsive" and asked to A/B raster vs vector. For a painterly scene the result was lopsided: **raster WebP = 163 KB, 1 sprite, 1 draw call, pixel-perfect**; **vtracer = 6.4 MB, 14,098 paths, AND posterised** (soft gradients collapse to flat polygons).
- **Why**: vector tracing models flat regions; painterly art is all gradients, so the tracer either floods with thousands of tiny polygons (huge + slow) or bands the gradients (ugly). Raster just IS the painting.
- **Lesson**: choose the asset format by the ART, not by instinct. Vector wins for flat/iconographic art (crisp at any zoom, tiny); raster wins for painterly/photographic art (smaller AND faithful). When unsure, measure bytes + path-count + a side-by-side — don't assume "vector = lighter". For a full-bleed game background, raster = one cover-fit Sprite (1 draw call) with multi-res tiers picked by `screen.width×dpr` is the responsive-and-accurate answer.

### L113 — Object-removal inpainting (LaMa) cleanly strips painted game elements from AI backdrops (A-312 v55.74)
- **Need**: the owner's AI-generated background mockups had the train + obstacles + UI painted INTO the scene; cropping removes the top/bottom UI bars but NOT the mid-scene train/obstacles (woven into rails + landscape).
- **Solution**: real inpainting. `simple-lama-inpainting` (pip) failed to build on Python 3.13, but the LaMa model is a standalone TorchScript file — download `big-lama.pt` (~206MB) and run it directly via `torch.jit.load` on the present CUDA. Inference = image+binary-mask tensors padded to mod-8, one forward pass, crop back. Mask = manual element rects per image + an auto HSV threshold for the distinctly-saturated props (orange cones / yellow stars), band-limited to the rail zone so it never masks rails/scenery.
- **Lesson**: when a pip wrapper won't install, the underlying model often runs directly (torchscript + a few lines of pre/post-processing). LaMa is excellent at "erase object, fill plausible background" even on painterly art. For a removal pipeline: generous masks are fine (LaMa refills convincingly); keep heavy intermediates (model, full-res plates) out of git, commit only the small mask recipes + the served downscaled assets. Verify each cleaned plate visually before shipping.

### L114 — Parallax depth from a single flat image = horizontal band-slice + per-band scroll speed (g14 v55.75)
- **Need**: a static painterly backdrop should feel like the train is travelling (3D parallax), but it is ONE flat image (no layer separation).
- **Solution**: slice the texture into horizontal DEPTH bands (`new PIXI.Texture({source, frame})` sub-rects), tile each 2× and scroll at a depth-scaled speed (far slow, near fast), wrapping at `-tileW`. The near ground rushing past + the horizon barely moving = convincing parallax from one image. Verify by measuring per-band x deltas (far shifts far less than near).
- **Lesson**: put band boundaries on natural ground lines (rails-top, foreground) so little vertical content crosses a cut (avoids tearing). The rail-bed band tiles seamlessly (rails repeat); scenery bands loop with mild repetition (fine for a runner). Sub-frame sprites share ONE base texture → cheap (~6-10 draw calls). Pin gameplay `laneYs` to the rail-bed band so the train rides the painted rails while the world scrolls. dt-clamp the scroll for the throttled tablet.

### L115 — A "rail bob" must move the WHOLE stage uniformly, not individual bands (g14 v55.76)
- **Need**: add a vertical 3-D "riding the rails" bounce to the flat-plate parallax without (a) inter-band seams, (b) the train floating off the painted rails, or (c) revealing a screen edge.
- **Trap**: bobbing each band's Y independently tears the seams between contiguous photographic bands AND, on the rail band, separates the painted rails from the fixed-lane train. Adding per-band vertical overlap margin then causes *ghosting* because adjacent bands scroll at different speeds.
- **Solution**: bob `stage.y` uniformly (sin + a small 2nd harmonic). Backdrop **and** train are both children of `stage`, so they move identically → zero seam, zero float. Clamp the amplitude to the cover-fit overscan (`-topY`, ≥0) so an edge is never exposed; it self-disables when the plate aspect ≈ screen (no overscan, e.g. portrait). Amplitude scales with speed (bouncier when fast). Reset `stage.y=0` on race start.

### L116 — Extract one verified engine into a shared module; migrate the source-of-truth game first + re-run its probe (v55.77)
- **Need**: the same painterly-backdrop parallax + speed overlay across three standalone PixiJS games (g14 top-down, g15 top-down lanes, g14-side side-scroller) without three diverging copies.
- **Solution**: lift g14's proven code into zero-dep globals `window.TrainBackdrop` (load/fit/mount/`tick(speed,dt,stage)`/refit + rail-bob) + `window.TrainSpeedFX` (always-on, speed-scaled streaks). **Migrate the originating game (g14) to the module FIRST and re-run its existing probe as the regression gate** — if it stays green, the extraction is faithful. Then adopt per-game: top-down games pin lanes to `manifest.laneRatios` and skip their procedural sky/scenery/track; the side-scroller mounts **scenery-only** (a `bandFilter` keeps sky+far bands `fy1<=0.6`, `railBob:false`) behind its own terrain so there are no clashing double-rails. Every call guarded → a fetch miss leaves each game's procedural background untouched.
- **Lesson**: PixiJS sub-frame textures + cover-fit + the speed var are the only per-game couplings — pass them as args, keep the module stateless-per-call. Probe gotcha: top-level `const`/`let` are lexical globals, NOT `window` properties, so a headless probe must read them by **bare identifier** (`typeof X !== 'undefined' ? X : null`), not `window.X`.

### L117 — Cover-fit backdrop sharpness on hi-dpi = a tier near the SOURCE width, not just lower quality (A-313 v55.78)
- **Need**: backdrops looked soft on an 11" tablet; owner asked to "compress a bit" without losing tablet quality.
- **Root**: a full-screen cover-fit backdrop on a dpr2 11" tablet needs ~`cssWidth*2` ≈ 2388 device px; the top tier was only 1280 (source plate 1672), so it upscaled 1.86× → blur. Lowering WebP quality makes files smaller but does NOTHING for the upscale blur.
- **Lesson**: add a tier near the source width (1600 for a 1672 plate) so hi-dpi devices stop upscaling; let the runtime pick the smallest tier ≥ `screen.width*dpr` (else the largest). Then trim the redundant mid-tiers + drop quality a notch (q82→q80, visually lossless for painterly art) to keep the TOTAL down. Net: tablet sharper AND the asset set lighter. Verify by sniffing which `*-NNNN.webp` the page actually requests at a simulated tablet viewport, not by eyeballing one screen.
