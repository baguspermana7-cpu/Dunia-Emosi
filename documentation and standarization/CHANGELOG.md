...
di plb.html, tidak hanya text tapi juga mock up visual." Built a NEW self-contained catalog page
`plb.html`:
- **1000 detailed ideas** (exactly) across the **14 non-train/non-Pokémon games** (Emosi G1/G2/G5, Huruf
  G3/G8/G9, Angka G4, Memori G7, Sains G11/G12/G25, Mobil G6/G20/G22), spread over 10 facets (mekanik,
  mode, tampilan, audio, progresi, edukasi, a11y, sosial, personalisasi, mini). Each idea = title +
  mekanik + gameplay + tampilan + nilai belajar + effort/impact, all in Indonesian, grounded in each
  game's real current mechanic. Generated by 14 parallel subagents (~1064 raw → deduped → balanced 1000).
- **Claymorphism UI matching the app** (Fredoka One + Nunito, clay-card recipe, pastel palette): hero +
  live count, sticky filter bar (chips per game + per facet + search, live count), lazy-rendered card
  grid (IntersectionObserver, 60/batch — no 1000-node dump).
- **14 flagship interactive mockups** (Canvas2D, no `ctx.filter` blur, mount-on-visible): animated
  emotion-face, breathing-ring, card-flip, phonics letter-pop, word-tile build, letter trace, counting
  pop, picture reveal, science lab, silhouette reveal, number-line hop, driving parallax, volley, candy
  catch — shown in a showcase gallery + openable from 84 flagged cards.
- Reproducible build: `tools/plb-data/g*.json` (source ideas) + `tools/plb-template.html` +
  `tools/build-plb.mjs` (merge/dedupe/trim/inject). Standalone design doc (like planb.html) — NOT wired
  into app nav or SW. Verified headless: 1000 ideas, filters narrow correctly, a mockup opens+renders,
  lazy-load 60→180 on scroll, **0 console errors** at 1280 + 390.
